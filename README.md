...
- **記憶體**：建議16GB以上
- **儲存空間**：至少5GB可用空間
- **GPU**：建議使用支援CUDA的GPU或Apple Silicon (M1/M2)
...
- **batch**: 12 - 批次大小
- **imgsz**: 640 - 輸入圖像尺寸
- **device**: 'mps' - 使用Apple Silicon GPU (macOS) 或 'cuda' (NVIDIA GPU)
- **patience**: 50 - 早停耐心值
...
### Q: 支援哪些設備？
A: 支援CUDA GPU、Apple Silicon (M1/M2)、CPU訓練。
